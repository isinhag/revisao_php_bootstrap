...
if (empty($dados['nome'])) {
    ...
    // Verificar se o usuário foi cadastrado com sucesso
        if ($cad_usuario->rowCount()) {
            ...
            // Recuperar o último ID inserido (ID do usuário recém-cadastro)
            $id_usuario = $conn->lastInsertId();

            // Cadastra no banco de dados na segunda tabela (tabela de endereos) associada ao usuário
            ...
            $cad_endereco->bindParam(':usuario_id', $id_endereco);
            $cad_endereco->execute();

            // Verificar se o endereço foi cadastrado com sucesso 
            if ($cad_endereco->rowCount()) {
                // Se ambos, usuários e endereço, forem cadastrados com sucesso, retorna mensagem de sucesso
                $retorna = ['status' => true, 'dados' => "<div class='alert alert-success' role='alert'>Usuário cadastrado com sucesso!</div>"];
            } else {
            ...
            // Se houver erro no cadastrar o usuário, retornar mensagem de erro
            $retorna = ['status' => false, 'msg' => "<div class='alert alert-danger' role='alert'>Erro: Usuário não cadastrado com sucesso!</div>"];
        }
}
